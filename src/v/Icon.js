...
class Icon extends Component {
	...
	//console.log("Icon["+this.props.name+"]color="+this.props.color+",margin="+JSON.stringify(margin))
        ...
        if(name.substring(0,2)==='mi')
            return <MIcon name={name.substring(3)} size={this.props.size} color={this.props.color} onPress={this.props.onPress} style={this.props.style,margin} />
    }
    renderBadge(badge){
        if(this.badge){
...
function getImageSource(name,size,color){
    ...
    if(name.substring(0,2)==='mi')  return MIcon.getImageSource(name.substring(3),size,color)
}
